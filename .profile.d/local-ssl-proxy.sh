...
function sslx() {
    if [ "$1" == "" ]; then
        ...
        ps -efA|grep -Eo 'node .+local-ssl-proxy.+\-\-source=[0-9]+ \-\-target=[0-9]+'|sed -E 's/.+\-\-source=//g'|sed -E 's/\-\-target=/-> /g'
    elif [ "$1" == "kill" ]; then
        ps -efA|grep -E "local-ssl-proxy.+\-\-source="$1|awk '{ print $2 }'|sudo xargs kill -9
    elif [ "$1" == "killall" ]; then
        ...
        ps -efA|grep -E "local-ssl-proxy.+\-\-source="$1|awk '{ print $2 }'|xargs kill -9
